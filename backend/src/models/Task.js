    ...
    hooks: {
      beforeCreate: async (task) => {
        ...
        if (!task.task_key && task.project_id) {
          const project = await sequelize.models.Project.findByPk(task.project_id);
          if (project) {
            const taskCount = await Task.count({ where: { project_id: task.project_id } });
      ...
      beforeUpdate: (task) => {
        ...
        if (task.changed('status') && task.status === 'done' && !task.completed_at) {
