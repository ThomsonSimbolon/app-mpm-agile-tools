...
exports.create = async (req, res, next) => {
  try {
    ...
    res
      .status(201)
      .json(
        formatResponse(true, "Task created successfully", { task: fullTask })
      );
  } catch (error) {
    next(error);
  }
...
exports.assign = async (req, res, next) => {
  try {
    ...
    res.json(
      formatResponse(true, "Task assigned successfully", { task: updatedTask })
    );
  } catch (error) {
    next(error);
  }
...
exports.createSubtask = async (req, res, next) => {
  try {
    ...
    if (!parentTask) {
      return res
        .status(404)
        .json(formatResponse(false, "Parent task not found"));
    }
    ...
    res
      .status(201)
      .json(formatResponse(true, "Subtask created successfully", { subtask }));
  } catch (error) {
    next(error);
  }
};
